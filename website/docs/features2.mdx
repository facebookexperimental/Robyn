...
#### Using calibration in Robyn

Robyn accepts a dataframe as calibration input in the `robyn_inputs()` function. An example dataframe is shown below. The function usage can be found in the [demo](https://github.com/facebookexperimental/Robyn/blob/main/demo/demo.R#L262).
...
Robyn automatically exports various graphical and tabular outputs into the specified path to aid result interpretation and further custom analysis.

### Model onepager

An onepager per pareto-optimal model is exported after running the `robyn_outputs()` function. An example:
