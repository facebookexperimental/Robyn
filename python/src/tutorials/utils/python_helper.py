...
hexToPng = lambda fileName, hexData: (
    Image.open(  # Open an image using the PIL library
        ...
        fileName, "png"
    )  # Save the opened image as a PNG file with the given fileName
)
...
def render_spendexposure(mmmdata_collection, max_size=(1000, 1500)):
    """
    Renders the exposure plots for the given MMMDataCollection.
    Args:
        mmmdata_collection: An MMMDataCollection object containing the model data.
    ...
    """
    print("MMMdata collection modNLS: ", mmmdata_collection.modNLS)

    if len(mmmdata_collection.exposure_vars) > 0:
        print("Rendering Spend vs Exposure Plots")
        for var in mmmdata_collection.exposure_vars:
            if var in mmmdata_collection.modNLS["plots"]:
                plot_data = mmmdata_collection.modNLS["plots"][var]
                img = Image(data=plot_data)
                display(img)
            else:
...
def plot_outputgraphs(OutputJson, argumenttype, graphtype, max_size=(1000, 1500)):
    ...
    if graphtype in ["moo_distrb_plot"]:
        image_data = binascii.unhexlify(
            "".join(OutputJson["convergence"]["moo_distrb_plot"])
        )
        ...
        image_data = binascii.unhexlify(
            "".join(OutputJson["convergence"]["moo_cloud_plot"])
        )
        image = Image.open(io.BytesIO(image_data))
        image.thumbnail(max_size, Image.Resampling.LANCZOS)
        display(image)
...
def load_onepager(
    ...
    if top_pareto == True and sol == "all":
        ...
        for i in range(len(OutputJson["clusters"]["models"])):
            ...
            onepager = load_modeldata(
                sol_id, InputJson=InputJson, OutputJson=OutputJson
            )
...
def write_robynmodel(sol, path, InputJson, OutputJson, OutputModels):
    ...
    updatedPath = create_robyn_directory(path)
    if sol in OutputJson["allSolutions"]:
        writeArgs = {"select_model": sol, "export": True, "dir": updatedPath}
