...
class HolidaysDataValidation(Validation):
    def __init__(self, holidays_data: HolidaysData) -> None:
        ...
        self.logger.debug("Initializing HolidaysDataValidation with holidays_data: %s", 
    ...
    def check_holidays(self) -> ValidationResult:
        ...
        self.logger.debug("Validating holidays DataFrame with shape: %s", dt_holidays.shape)
        error_details = {}
        error_message = ""
        ...
        if na_cols:
            ...
            for col in na_cols:
                ...
                self.logger.warning("Column '%s' contains %d missing values (%.2f%%)",
        ...
        if missing_cols:
            self.logger.error("Required columns missing in holidays data: %s", missing_cols)
        ...
        if invalid:
            self.logger.error("Invalid column names found (contains spaces): %s", invalid)
        ...
        self.logger.info("Holidays validation completed. Status: %s", validation_result.status)
    ...
    def check_prophet(self) -> ValidationResult:
        ...
        self.logger.debug("Prophet validation parameters - vars: %s, signs: %s, country: %s",
                         prophet_vars, prophet_signs, prophet_country)
        ...
        try:
            if dt_holidays is None or prophet_vars is None:
                self.logger.info("No holidays data or prophet variables provided, skipping validation")
                return ValidationResult(status=True, error_details={}, error_message="")
            ...
            if ProphetVariableType.HOLIDAY in prophet_vars:
                self.logger.debug("Validating prophet_country against available countries")
                if prophet_country is None:
                    self.logger.error("prophet_country is None but HOLIDAY type is specified in prophet_vars")
                elif prophet_country not in dt_holidays['country'].unique():
                    available_countries = dt_holidays['country'].unique()
                    self.logger.error("Invalid prophet_country '%s'. Available countries: %s",
                                    prophet_country, available_countries)
                    error_details['prophet_country'] = f"Invalid prophet_country. Available countries: {', '.join(available_countries)}"
            ...
            if len(prophet_signs) == 1:
                self.logger.debug("Expanding single prophet_sign to match prophet_vars length")
                prophet_signs = prophet_signs * len(prophet_vars)

            if len(prophet_signs) != len(prophet_vars):
                self.logger.error("Length mismatch: prophet_signs (%d) != prophet_vars (%d)",
                                len(prophet_signs), len(prophet_vars))
            ...
            self.logger.info("Prophet validation completed. Status: %s", validation_result.status)
            return validation_result

        except Exception as e:
            self.logger.error("Prophet validation failed with error: %s", str(e), exc_info=True)
    ...
    def validate(self) -> List[ValidationResult]:
        ...
        self.logger.info("Validation complete. Overall status: %s",
