...
class HyperparametersValidation(Validation):
    ...
    def __init__(self, hyperparameters: Hyperparameters) -> None:
        ...
        logger.debug("Initialized HyperparametersValidation with hyperparameters: %s", 
    ...
    def check_hyperparameters(self) -> ValidationResult:
        ...
        logger.info("Hyperparameter validation completed. Status: %s", validation_result.status)
        if error_message:
    ...
    def check_train_size(self):
        ...
        if lower < 0.1 or lower >= upper or upper > 1:
            logger.error("Train_size values out of bounds: lower=%s, upper=%s", lower, upper)
    ...
    def hyper_names(self, all_media: List[str]) -> List[str]:
        ...
        """
        logger.debug("Generating hyperparameter names for adstock type: %s", self.hyperparameters.adstock)
    ...
    def check_hyper_limits(self, hyper: str) -> None:
        ...
        logger.debug("Parsed limits - lower: %s (%s), upper: %s (%s)", 
                    lower_limit, lower_inclusive, upper_limit, upper_inclusive)
        ...
        for channel, channel_hyperparams in self.hyperparameters.hyperparameters.items():
            ...
            if len(values) not in [1, 2]:
                logger.error("Invalid number of values for %s in channel %s: %s", 
                           hyper, channel, len(values))
            ...
            (not lower_inclusive and float(values[0]) <= lower_limit):
                logger.error("Lower bound violation for %s in channel %s: %s", 
                           hyper, channel, values[0])
            ...
            if len(values) == 2:
                ...
                (not upper_inclusive and float(values[1]) >= upper_limit):
                    logger.error("Upper bound violation for %s in channel %s: %s", 
                               hyper, channel, values[1])
                    raise ValueError(f"{hyper}'s hyperparameter must be {'≤' if upper_inclusive else '<'} {upper_limit}")
                if float(values[0]) > float(values[1]):
                    logger.error("Invalid bounds order for %s in channel %s: %s", 
                               hyper, channel, values)
                    raise ValueError(f"{hyper}'s hyperparameter must have lower bound first and upper bound second")
            
            logger.debug("Validation passed for %s in channel %s: %s", hyper, channel, values)
    ...
    def validate(self) -> List[ValidationResult]:
        ...
        logger.info("Validation completed with status: %s", 
