...
class MMMDataValidation(Validation):
    def __init__(self, mmm_data: MMMData) -> None:
        ...
        logger.debug("Initializing MMMDataValidation with data shape: %s", self.mmm_data.data.shape)
    ...
    def check_variable_names(self) -> ValidationResult:
        ...
        if invalid:
            ...
            logger.warning("Found invalid variable names (containing spaces): %s", invalid)
        ...
        if date_var not in data.columns:
            ...
            try:
                ...
                if not data[date_var].is_monotonic_increasing:
                    ...
                    logger.warning("Date variable validation failed: dates not in ascending order")
            except ValueError:
    ...
    def validate(self) -> List[ValidationResult]:
        ...
        if failed_validations:
            logger.error("Validation completed with %d failures", len(failed_validations))
        else:
