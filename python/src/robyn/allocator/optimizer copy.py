...
class BudgetAllocator:
    ...
    def _run_optimization(
        ...
        constraints = []
        if self.params.scenario == SCENARIO_TARGET_EFFICIENCY:
            # Add target efficiency constraint
            ...
            # Add budget constraint for max_response scenario
            constraints.append(
                {
        ...
        budget_violation = (
            abs(np.sum(final_spend) - self.constraints.budget_constraint)
            if self.params.scenario == SCENARIO_MAX_RESPONSE
            else 0
        )
    ...
    def _objective_function(self, x: np.ndarray) -> float:
        """Modified objective function with target efficiency handling."""
        responses = np.zeros(len(self.media_spend_sorted))
        for i, spend in enumerate(x):
            responses[i] = self.calculate_response(spend, i)
        ...
        if self.params.scenario == SCENARIO_TARGET_EFFICIENCY:
            if self.dep_var_type == "revenue":
                # For ROAS: maximize response while maintaining minimum ROAS
                ...
                # For CPA: minimize spend while maintaining maximum CPA
                actual_cpa = (
            ...
            # Original max_response objective
    ...
    def _setup_target_efficiency_constraints(self) -> Constraints:
        ...
        logger.debug("Setting up target efficiency constraints...")
        ...
        if self.params.target_value is None:
            if self.dep_var_type == "revenue":
                ...
                logger.debug(f"Initial ROAS: {initial_roas:.4f}")
                logger.debug(f"Target ROAS: {target_value:.4f}")
            else:
                initial_cpa = np.sum(self.init_spend_unit) / np.sum(self.init_response)
                target_value = initial_cpa * 1.2  # Target 120% of initial CPA
                logger.debug(f"Initial CPA: {initial_cpa:.4f}")
                logger.debug(f"Target CPA: {target_value:.4f}")
        else:
            target_value = self.params.target_value

        logger.debug("\nTarget Efficiency Constraints:")
        logger.debug(f"Target value: {target_value:.4f}")
        logger.debug("\nChannel bounds:")
        for channel, lb, ub in zip(self.media_spend_sorted, lower_bounds, upper_bounds):
            logger.debug(f"{channel}:")
            logger.debug(f"  Lower: {lb:,.2f}")
            logger.debug(f"  Upper: {ub:,.2f}")
