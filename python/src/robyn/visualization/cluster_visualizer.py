...
class ClusterVisualizer:
    ...
    def plot_confidence_intervals(
        ...
        for idx, (cluster, ax) in enumerate(zip(unique_clusters, axes)):
            ...
            for i, density in enumerate(data_matrix):
                ...
                # Fill area
                ax.fill_between(x_range, y_base, y_base + density, alpha=0.5, color="#4682B4")  # Steel blue color
        ...
        fig.suptitle(f"In-Cluster {temp} & Bootstrapped 95% CI\n" "Sampling distribution of cluster mean", y=0.95)
    ...
    def plot_top_solutions_errors(
        ...
        sns.scatterplot(data=merged_df[~merged_df["highlight"]], x="nrmse", y="decomp.rssd", color="gray")
        sns.scatterplot(
            ...
            palette=sns.color_palette(
                "husl", n_colors=len(merged_df.loc[merged_df["highlight"], "label"].unique()), as_cmap=False
            ),
        )
    ...
    def plot_topsols_rois(self, df: pd.DataFrame, top_sols: pd.DataFrame, all_media: list[str]) -> Figure:
        """
        ...
        merged_df["label"] = merged_df.apply(lambda row: f'[{row["cluster"]}:{row["rank"]}] {row["sol_id"]}', axis=1)
        # Melt the DataFrame
        columns_to_melt = [col for col in merged_df.columns if any(media in col for media in all_media)]
        melted_df = pd.melt(
            merged_df, id_vars=["label"], value_vars=columns_to_melt, var_name="media", value_name="perf"
        )
    ...
    def generate_bootstrap_confidence(self, ax: Optional[plt.Axes] = None) -> Optional[plt.Figure]:
        ...
        bootstrap_data = x_decomp_agg[(~x_decomp_agg["ci_low"].isna()) & (x_decomp_agg["solID"] == model_id)][
        ...
        for i, row in enumerate(bootstrap_data.itertuples()):
            ...
            # CI values
            ax.text(row.ci_low, i, f"{float(f'{row.ci_low:.2g}')}", va="center", ha="right", fontsize=9, color="black")

            ax.text(row.ci_up, i, f"{float(f'{row.ci_up:.2g}')}", va="center", ha="left", fontsize=9, color="black")
