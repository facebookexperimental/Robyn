from robyn.visualization.codegen.base_visualizer_revised import BaseVisualizer
...
class AllocationPlotter(BaseVisualizer):
    ...
    def plot_spend_allocation(self) -> plt.Figure:
        ...
        ax.legend()
        for idx, (curr, opt) in enumerate(zip(current_spend, optimal_spend)):
            percent_change = ((opt / curr) - 1) * 100
            color = self.positive_color if percent_change >= 0 else self.negative_color
            ax.text(
                x[idx],
                max(curr, opt) + 0.05 * max(curr, opt),
                f"{percent_change:.1f}%",
                color=color,
                ha="center",
            )
    ...
    def plot_efficiency_frontier(self) -> plt.Figure:
        ...
        ax.annotate(
            f"Spend Change: {pct_spend_change:.2f}%\nResponse Change: {pct_response_change:.2f}%",
    ...
    def plot_spend_vs_response(self) -> plt.Figure:
        ...
        colors = [
            self.negative_color if pct < 0 else self.positive_color for pct in spend_pct
        ]
        ...
        colors = [
            self.negative_color if pct < 0 else self.positive_color
    ...
    def plot_summary_metrics(self) -> plt.Figure:
        ...
        optimal_allocations = self.result.optimal_allocations
        channels = optimal_allocations.index
        ...
        for i, channel in enumerate(channels):
            curr = current_metric[i]
            opt = optimal_metric[i]
