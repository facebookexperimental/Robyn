...
class ModelConvergenceVisualizer:
    def __init__(self, n_cuts: int, nrmse_win: List[float]):
        ...
        logger.info("Initialized ModelConvergenceVisualizer with n_cuts=%d, nrmse_win=%s", n_cuts, nrmse_win)
    ...
    def create_moo_distrb_plot(self, dt_objfunc_cvg: pd.DataFrame, conv_msg: List[str]) -> str:
        ...
        try:
            ...
            for error_type in dt_objfunc_cvg["error_type"].unique():
                ...
                logger.debug("Clipped values for error_type %s: min=%f, max=%f", 
                           error_type, quantiles[0], quantiles[1])

            fig, ax = plt.subplots(figsize=(12, 8))
            sns.violinplot(data=dt_objfunc_cvg, x="value", y="cuts", 
                         hue="error_type", split=True, inner="quartile", ax=ax)
    ...
    def create_moo_cloud_plot(self, df: pd.DataFrame, conv_msg: List[str], calibrated: bool) -> str:
        logger.debug("Starting moo cloud plot creation with data shape: %s, calibrated=%s", 
                    df.shape, calibrated)
        ...
        try:
            ...
            scatter = ax.scatter(df["nrmse"], df["decomp.rssd"], 
                               c=df["ElapsedAccum"], cmap="viridis")
    ...
    def create_ts_validation_plot(self, trials: List[Trial]) -> str:
        ...
        try:
            ...
            # Train Size plot
            sns.scatterplot(data=result_hyp_param, x="iteration", y="train_size", 
                          hue="trial", ax=ax2, legend=False)
