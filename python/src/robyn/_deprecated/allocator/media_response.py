...
class MediaResponseParamsCalculator:
    ...
    def __init__(
        ...
        logger.debug(
            ...
            type(mmm_data).__name__,
            type(pareto_result).__name__,
            select_model,
        )
        ...
        logger.debug(
            "Initialized with %d media channels: %s",
            len(self.sorted_channels),
            self.sorted_channels.tolist(),
        )

    def calculate_parameters(self) -> MediaResponseParameters:
        """Calculate response parameters with improved error handling."""
        logger.info(
            "Starting media response parameters calculation for model %s",
            self.select_model,
        )
        ...
        try:
            ...
            model_key = (
                "solID"
                if "solID" in self.pareto_result.result_hyp_param.columns
                else "sol_id"
            )
            ...
            if dt_hyppar.empty:
                ...
                raise ValueError(
                    f"No hyperparameters found for model {self.select_model}"
                )
            ...
            for channel in self.sorted_channels:
                ...
                if alpha_key not in dt_hyppar.columns:
                    logger.warning(
                        "Missing %s, using default alpha value of 1.0", alpha_key
                    )
                    alphas[channel] = 1.0
                else:
                ...
                if gamma_key not in dt_hyppar.columns:
                    logger.warning(
                        "Missing %s, using default gamma value of 0.5", gamma_key
                    )
                    gamma = 0.5
                else:
                ...
                if channel in self.mmm_data.data.columns:
                    ...
                    logger.debug(
                        "Channel %s inflexion: %f", channel, inflexions[channel]
                    )
                else:
                    logger.warning(
                        "Channel %s not found in data, using default inflexion", channel
                    )
                ...
                coef_data = self.pareto_result.x_decomp_agg[
                    (
                        self.pareto_result.x_decomp_agg[model_key_decomp]
                        == self.select_model
                    )
                ...
                if coef_data.empty:
                    logger.warning(
                        "No coefficient data found for %s, using default coefficient of 1.0",
                        channel,
                    )
                    ...
                    logger.debug(
                        "Channel %s coefficient: %f", channel, coefficients[channel]
                    )

            result = MediaResponseParameters(
                alphas=alphas, inflexions=inflexions, coefficients=coefficients
            )
        ...
        try:
            ...
            if hasattr(self.pareto_result, "media_vec_collect"):
                ...
                if not isinstance(media_vec, pd.DataFrame) or media_vec.empty:
                    ...
                    logger.debug(
                        "Available columns in media_vec_collect: %s",
            ...
            end_idx = getattr(
                self.mmm_data.mmmdata_spec, "rolling_window_end_which", len(raw_data)
            )

            logger.debug(
                "Using data window: start_idx=%d, end_idx=%d", start_idx, end_idx
            )
