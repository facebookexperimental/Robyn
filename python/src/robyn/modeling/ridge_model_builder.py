...
class RidgeModelBuilder:
    ...
    def _calculate_rssd(
        ...
        self.logger.debug(f"Y scale - mean: {y_mean:.2e}, sd: {y_sd:.2e}")
        ...
        for col in paid_media_cols:
            ...
            self.logger.debug(f"  Coef (raw): {coef:.6e}")
            self.logger.debug(f"  Coef (unscaled): {unscaled_coef:.6e}")
            self.logger.debug(f"  Effect: {effect:.6e}")
            self.logger.debug(f"  Spend: {spend:.6e}")
        ...
        self.logger.debug(f"\nRSSD summary:")
        self.logger.debug(f"Total effect: {total_effect:.2e}")
        self.logger.debug(f"Total spend: {total_spend:.2e}")
        self.logger.debug(f"Number of zeros: {zero_effects}")
        self.logger.debug(f"Base RSSD: {rssd/scale_factor:.6f}")
        self.logger.debug(f"Scaled RSSD: {rssd:.6f}")
        self.logger.debug(f"Final RSSD: {rssd * (1 + zero_ratio if rssd_zero_penalty else 1):.6f}")
