...
class ParetoOptimizer:
    ...
    def _aggregate_model_data(self, calibrated: bool) -> Dict[str, pd.DataFrame]:
        ...
        self.model_outputs.trials = [
            self._ensure_trial_ids(trial) for trial in self.model_outputs.trials
        ]
        ...
        if (
            len(xDecompAgg["sol_id"].unique()) == 1
    ...
    def run_dt_resp(self, row: pd.Series, paretoData: ParetoData) -> pd.Series:
        ...
        try:
            ...
            response_output: ResponseOutput = (
                response_curve_calculator.calculate_response(
    ...
    def _compute_response_curves(
        ...
        try:
            ...
            print(
                "Pareto data decomp_spend_dist columns: ",
                pareto_data.decomp_spend_dist.columns,
            )
            print(
                "Pareto Data Mean Spend: ", pareto_data.decomp_spend_dist["mean_spend"]
            )
            print(
                "Pareto Data Mean Response: ",
        ...
        for pf in pareto_fronts_vec:
            ...
            for sid in tqdm(uniqueSol, desc="Processing Solutions", unit="solution"):
                try:
                    ...
                    plotWaterfallLoop["end"] = (
                        1 - plotWaterfallLoop["xDecompPerc"].cumsum()
                    )
                    plotWaterfallLoop["start"] = (
                        plotWaterfallLoop["end"].shift(1).fillna(1)
                    )
                    ...
                    plotWaterfallLoop["sign"] = pd.Categorical(
                        np.where(
                            plotWaterfallLoop["xDecompPerc"] >= 0,
                            "Positive",
                            "Negative",
                        )
                    )
                    ...
                    print(
                        "resultHypParamLoop columns:",
                        resultHypParamLoop.columns.tolist(),
                    )
                    print("resultHypParamLoop sample:\n", resultHypParamLoop.head())
                    missing_columns = [
                        col
                        for col in get_hp_names
                        if col not in resultHypParamLoop.columns
                    ]
                    if missing_columns:
                    ...
                    if self.hyperparameter.adstock == AdstockType.GEOMETRIC:
                        ...
                        for v1 in range(len(self.mmm_data.mmmdata_spec.all_media)):
                            ...
                            max_non0 = (
                                dt_weibull["decay_accumulated"] > 0.001
                            ).argmax()
                            dt_weibull["cut_time"] = (
                                max_non0 * 2
                                if max_non0 <= 5
                    ...
                    for med in range(len(all_media_channels)):
                        ...
                        if adstock == AdstockType.GEOMETRIC:
                            thetas = hypParam[
                                f"{all_media_channels[med]}_thetas"
                            ].values
                            ...
                            shapes = hypParam[
                                f"{all_media_channels[med]}_shapes"
                            ].values
                            scales = hypParam[
                                f"{all_media_channels[med]}_scales"
                            ].values
                    ...
                    xDecompVec = (
                        xDecompAgg[xDecompAgg["sol_id"] == sid][
                            ["sol_id", "rn", "coef"]
                        ]
                    ...
                    mediaVecCollect = pd.concat(
                        [
                            ...
                            dt_transformAdstock.assign(
                                type="adstockedMedia", sol_id=sid
                            ),
                            dt_transformSaturation.assign(
                                type="saturatedMedia", sol_id=sid
                            ),
