...
class ResponseCurveCalculator:
    def __init__(
        ...
        self.transformation = Transformation(mmm_data)  # Pass the MMMData instance
    ...
    def _check_metric_dates(
        ...
        if not quiet:
            print(
                f"Using date range: {date_range_updated.iloc[0]} to {date_range_updated.iloc[-1]}"
            )

        return MetricDateInfo(
    ...
    def _check_metric_value(
        ...
        if metric_value is None:
            ...
            if len(metric_value_updated) != len(all_values[metric_loc]):
                raise ValueError(
                    f"Length of metric_value ({len(metric_value_updated)}) does not match the selected date range ({len(all_values[metric_loc])})"
                )
