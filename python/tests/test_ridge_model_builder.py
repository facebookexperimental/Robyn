...
def sample_mmmdata():
    ...
    # Create an instance of MMMDataSpec
    mmmdata_spec = MMMData.MMMDataSpec(
        ...
        paid_media_vars=None,  # Add more details if necessary
        paid_media_signs=[PaidMediaSigns.POSITIVE] * 3,  # Assuming positive impact
        organic_vars=None,  # Add if there are any organic variables
        organic_signs=None,  # Add corresponding signs if there are organic variables
        context_vars=None,  # Add if there are any context variables
        context_signs=None,  # Add corresponding signs if there are context variables
        factor_vars=None,  # Add if there are any factor variables
    )
    # Return an instance of MMMData
...
def test_prepare_data(ridge_model_builder, adstock_type):
    ...
    X, y = ridge_model_builder._prepare_data(hyperparams)
...
def test_run_nevergrad_optimization(mock_ng_registry, ridge_model_builder):
    ...
    mock_optimizer.ask.return_value = Mock(value=np.array([0.5, 0.5, 0.5]))
