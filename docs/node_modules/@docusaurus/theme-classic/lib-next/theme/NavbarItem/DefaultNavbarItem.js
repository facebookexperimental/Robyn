...
function NavItemMobile({
  ...
  const [collapsed, setCollapsed] = useState(() => !items?.some(item => isSamePath(item.to, pathname)) ?? true);

  const navLinkClassNames = (extraClassName, isSubList = false) => clsx('menu__link', {
  ...
  const menuListHeight = menuListRef.current?.scrollHeight ? `${menuListRef.current?.scrollHeight}px` : undefined;
  return <li className={clsx('menu__list-item', {
    'menu__list-item--collapsed': collapsed
  })}>
      <NavLink role="button" className={navLinkClassNames(className, true)} {...props} onClick={() => {
