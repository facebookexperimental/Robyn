...
function getNavbarTranslationFile(navbar) {
  ...
  function flattenNavbarItems(items) {
    const subItems = (0, _lodash.flatten)(items.map(item => {
      var _item$items;

      const allSubItems = (0, _lodash.flatten)([(_item$items = item.items) !== null && _item$items !== void 0 ? _item$items : []]);
...
function translateNavbar(navbar, navbarTranslations) {
  ...
  return { ...navbar,
    ...
    items: navbar.items.map(item => {
      ...
      return { ...item,
        label: (_navbarTranslations$$ = (_navbarTranslations$ = navbarTranslations[`item.label.${item.label}`]) === null || _navbarTranslations$ === void 0 ? void 0 : _navbarTranslations$.message) !== null && _navbarTranslations$$ !== void 0 ? _navbarTranslations$$ : item.label
      };
    })
  };
}
...
function getFooterTranslationFile(footer) {
  // TODO POC code
  const footerLinkTitles = (0, _lodash.chain)(footer.links.filter(link => !!link.title)).keyBy(link => `link.title.${link.title}`).mapValues(link => ({
    message: link.title,
    description: `The title of the footer links column with title=${link.title} in the footer`
  })).value();
  const footerLinkLabels = (0, _lodash.chain)((0, _lodash.flatten)(footer.links.map(link => link.items)).filter(link => !!link.label)).keyBy(linkItem => `link.item.label.${linkItem.label}`).mapValues(linkItem => {
    var _linkItem$to;
...
function translateThemeConfig({
  ...
  return { ...themeConfig,
    ...
    footer: translateFooter(themeConfig.footer, translationFilesMap.footer.content)
  };
}