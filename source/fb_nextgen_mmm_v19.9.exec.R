...
#############################################################################################
####################    Facebook MMM Open Source 'Robyn' Beta - V19.8  ######################
####################                    2020-09-25                     ######################
#############################################################################################
...
source(paste(script_path, "fb_nextgen_mmm_v19.9.func.R", sep=""))
source(paste(script_path, "fb_nextgen_mmm_v19.9.plot.R", sep=""))
source(paste(script_path, "fb_nextgen_mmm_v19.9.optm.R", sep=""))
...
# f.getProphet(dt_input) # test and visualise prophet decomposition
...
## set training size
f.plotTrainSize(T) # insert TRUE to plot training size guidance. Please balance between higher Bhattacharyya coefficient and sufficient training size
set_modTrainSize <- 0.7 # 0.7 means taking 70% of data to train and 30% to test the model

## set model core features
adstock <- "weibull" # geometric or weibull . weibull is more flexible, yet has one more parameter and thus takes longer
set_iter <- 100 # We recommend to run at least 100k iteration

# no need to change
f.plotAdstockCurves(T)
set_hyperBoundGlobal <- list(thetas = c(0, 0.5) # geometric decay rate
                          ,shapes = c(0.0001, 2) # weibull parameter that controls the decay shape between exponential and s-shape
                          ,scales = c(0, 0.05) # weibull parameter that controls the position of inflection point
                          ,alphas = c(0.0001, 3) # hill function parameter that controls the shape between exponential and s-shape
                          ,gammas = c(0.0001, 1) # hill functionn pararmeter that controls the scale of trarnsforrmation 
...
## each bound can be either a range (e.g c(0.1,0.9)) or one fixed value

set_hyperBoundLocal <- list(
  facebook_I_alphas = c(0.0001, 3)
  ,facebook_I_gammas = c(0.0001, 1)
  ,facebook_I_thetas = c(0.86, 0.94)
  #,facebook_I_scales = c(0.2)
  #,facebook_I_shapes = c(0.0001, 1)
  ,ooh_S_alphas = c(0.0001, 3)
  ,ooh_S_gammas = c(0.0001, 1)
  ,ooh_S_thetas = c(0, 0.9999)
  #,ooh_S_scales = c(0.1, 0.4)
  #,ooh_S_shapes = c(0.1, 2)
  ,print_S_alphas = c(0.3, 0.61)
  ,print_S_gammas = c(0.0001, 1)
  ,print_S_thetas = c(0, 0.9999)
  #,print_S_scales = c(0.4)
  #,print_S_shapes = c(0.1, 2)
  ,tv_S_alphas = c(2.6, 3)
  ,tv_S_gammas = c(0.0001, 1)
  ,tv_S_thetas = c(0.179, 0.504)
  #,tv_S_scales = c(0.0001, 0.45)
  #,tv_S_shapes = c(0.0001, 2)
  ,search_clicks_P_alphas = c(0.1, 3)
  ,search_clicks_P_gammas = c(0.1, 0.9)
  ,search_clicks_P_thetas = c(0.2, 0.8)
  #,search_clicks_P_scales = c(0.0001, 0.38)
...
bestAdstock <- f.plotMediaTransform(F, channelPlot = set_mediaVarName) # plot best model media transformation with 3 plots: adstock decay rate, adstock effect & response curve. Max. 3 channels per plot
...
optim_result <- f.optimiser(scenario = "max_historical_response" # c(max_historical_response, max_response_expected_spend)
                            #,expected_spend = 20000000 # specify future spend volume. only applies when scenario = "max.response.expected.spend"
                            #,expected_spend_days = 365 # specify period for the future spend volumne in days
                            ,channel_constr_low = c(0.7, 0.75, 0.60, 0.5, 0.65) # must be same length between 0.01-1 and has same length and order as set_mediaVarName
                            ,channel_constr_up = c(1.2, 1.5, 1.5, 3, 3)
                            ) # must be same length between 1-5 and has same length and order as set_mediaVarName
