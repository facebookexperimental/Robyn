...
f.optimiser <- function(optim_algo = "MMA_AUGLAG"
                        ,expected_spend = NULL
  ...
  if (exists("best_model")) {
    ...
    ## get dt 
    dt_bestHyperParam <- best_model$resultCollect$resultHypParam
    dt_bestCoef <- best_model$resultCollect$xDecompAgg
    dt_optim <- copy(dt_mod)
    
    ## get filter for channels mmm coef reduced to 0
    dt_coef <- best_model$resultCollect$xDecompAgg[rn %in% set_mediaVarName, .(rn, coef)]
    ...
    adstockMultiplierVec <- colSums(chnAdstocked) / colSums(dt_optimCost)
    ...
    stop("must run f.mmm.out function first to get output best_model")
  }
  
  ## collect output 
  
  dt_bestModel <- best_model$resultCollect$xDecompAgg[rn %in% channelNames, .(rn, spend, xDecompAgg, roi)][order(rn)]
