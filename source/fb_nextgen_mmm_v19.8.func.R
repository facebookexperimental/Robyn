  ...
  if (any(costSelector)) {
    ...
    for (i in 1:mediaVarCount) {
      if (costSelector[i]) {
        ...
        # estimate starting values for nls using lm
        modLM <- lm(log(reach) ~ spend, dt_spendModInput)
        nlsStartVal <- list(Vmax = exp(coef(modLM)[1]), Km = coef(modLM)[2]) 
        
        # run nls model 
        modNLS <- nls(reach ~ Vmax * spend/(Km + spend), #Michaelis-Menten model Vmax * spend/(Km + spend)
                       data = dt_spendModInput,
                       start = nlsStartVal 
        ...
        yhatNLSQA <- modNLSSum$coefficients['Vmax',1] * dt_spendModInput$spend / (modNLSSum$coefficients['Km',1] + dt_spendModInput$spend)
        ...
        modNLSCollect[[set_mediaVarName[i]]] <- data.table(channel = set_mediaVarName[i],
                                                             Vmax = modNLSSum$coefficients['Vmax',1],
                                                             Km = modNLSSum$coefficients['Km',1],
                                                             aic_nls = AIC(modNLS),
                                                             aic_lm = AIC(modLM),
                                                             bic_nls = BIC(modNLS),
                                                             bic_lm = BIC(modLM),
